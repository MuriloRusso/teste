...
class UserController extends Controller
{
    public function index(Request $request)
    {
        ...
        $users = ModelsProduct::all();

        // Finalizando rifas vencidas ou que foram vendidas 100%
        // foreach ($rifas->where('status', '=', 'Ativo') as $value) {
        //     if ($value->numbers()->count() == $value->qtdNumerosPagos()) {
        //         $value->status = 'Finalizado';
        //         $value->update();
        //     } else if ($value->draw_date != null && $value->draw_date <= date('Y-m-d H:i:s')) {
        //         $value->status = 'Finalizado';
        //         $value->update();
        //     }
        // }
        ...
        ->select('users.id', 'users.name', 'users.telephone', 'users.email', 'users.status')
        ->get();
        ...
        //     $cleanNumbers[] = UserController::getRafflesNumbers($_ProductID);
        // endforeach;
        // $getArrayCleaner = UserController::cleanEmptyArrays($cleanNumbers);
       ...
       // return view('User', ['users' => $users]);
        ...
        // return view('nome_da_view', ['users' => $users]);
...
/*
    ...
    public function getRafflesNumbers($productID)
    {
        ...
        foreach ($_ObjectParticipant as $keyParticipant => $valParticipant) :
            if ($valParticipant->product_id == $productID) :
                ...
                $countRafflesByProduct[$productID]['participants'][$valParticipant->cpf]['numbres'][$productID]['reservado'][] = UserController::getRafflesByID($valParticipant->raffles_id, $productID)->reservado;

                $countRafflesByProduct[$productID]['participants'][$valParticipant->cpf]['numbres'][$productID]['pago'][] = UserController::getRafflesByID($valParticipant->raffles_id, $productID)->pago;
            endif;
        endforeach;
    ...
    public function releaseReservedRafflesNumbers(Request $request)
    {
        ...
        //     if(UserController::updateReservationsToAvailable($valNumbers->id)){
        //         $count++;
        //     }
        // endforeach;
    ...
    public function resumoPDF($id)
    {
        ...
        $dompdf->stream('Resumo ' . $rifa->name . '.pdf');
    }
    ...
    public function comprasBusca($idRifa, Request $request)
    {
        ...
        if ($request->cota) {
            $cota = Raffle::where('product_id', '=', $idRifa)->where('number', '=', $request->cota)->first();
            if ($cota && $cota->participant_id != null) {
                $participantes = Participante::where('id', '=', $cota->participant_id)->get();
            } else {
    ...
    public function criarCompra(Request $request)
    {
        ...
        try {
            if ($rifa->modo_de_jogo == 'numeros') {
                ...
                $totalCompra = count($resultNumbers) * $valorNumero;

                $numbersRifa = $rifa->numbers();

                $selecionados = [];
                foreach ($resultNumbers as $key => $value) {
                    $expl = explode("-", $value);
                    $keyNumber = end($expl);
