...
<form action="{{ route('informarGanhadores') }}" method="POST">
    @csrf
        ...
        @foreach ($rifa->premios()->where('descricao', '!=', '') as $premio)
            <div class="col-md-12 mt-2">
                <div class="input-group">
                    ...
                    <input type="text" min="0" max="{{ $rifa->qtd - 1 }}" placeholder="Informe o numero da cota" class="form-control" name="cotas[{{ $premio->ordem }}]" required>
                </div>
            </div>
        @endforeach
    </div>
